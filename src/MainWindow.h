...
private slots:
  ...
  void onToggleContourRepairMode(bool on);
  void onContourRepairPointClicked(const QPointF& p);
  void onDeleteRegion();
...
private:
 ...
 // bool pose_on_=false;
  ...
  QPushButton* btnContourRepairMode_=nullptr;
  ...
  bool contour_repair_enabled_ = false;
  int contour_repair_contour_index_ = -1;
  std::vector<cv::Point> contour_repair_path_;
  std::vector<cv::Point> contour_repair_snap_points_;
