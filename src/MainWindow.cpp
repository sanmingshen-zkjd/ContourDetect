...
void MainWindow::updateLeftVisualDashboard() {
  ...
  auto fill=[&](QCustomPlot* p, int metricIdx, const QString& yLabel){
    ...
    if (curIdx >= 0) {
      ...
      std::vector<int> idsForLabels;
      if (selected_target_id_ >= 0 && showIds.count(selected_target_id_)) idsForLabels.push_back(selected_target_id_);
      else idsForLabels.assign(showIds.begin(), showIds.end());
