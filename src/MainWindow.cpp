...
void MainWindow::buildUI() {
    ...
    auto mkPlot=[&](const QString& y){
      ...
      connect(p, &QCustomPlot::mouseDoubleClick, this, [p](QMouseEvent*){ p->rescaleAxes(true); p->replot(); });
    ...
    QSplitter* srcSplit = new QSplitter(Qt::Horizontal, tabCap);
    QWidget* leftSrcPane = new QWidget(srcSplit);
    QVBoxLayout* leftSrcLay = new QVBoxLayout(leftSrcPane);
    leftSrcLay->setContentsMargins(0,0,0,0);
    leftSrcLay->setSpacing(6);
    leftSrcLay->addWidget(btnAddVideo_);
    leftSrcLay->addWidget(btnAddImgSeq_);
    ...
    leftSrcLay->addStretch(1);

    QWidget* rightSrcPane = new QWidget(srcSplit);
    QVBoxLayout* rightSrcLay = new QVBoxLayout(rightSrcPane);
    rightSrcLay->setContentsMargins(0,0,0,0);
    rightSrcLay->setSpacing(6);
    rightSrcLay->addWidget(new QLabel("Data Directory", rightSrcPane));
    fsModel_ = new QFileSystemModel(rightSrcPane);
    fsModel_->setFilter(QDir::AllDirs | QDir::NoDotAndDotDot | QDir::Files);
    fsModel_->setRootPath(QDir::homePath());
    treeSourceFs_ = new QTreeView(rightSrcPane);
    ...
    rightSrcLay->addWidget(treeSourceFs_, 1);

    srcSplit->addWidget(leftSrcPane);
    srcSplit->addWidget(rightSrcPane);
    srcSplit->setStretchFactor(0, 2);
    srcSplit->setStretchFactor(1, 3);
    capv->addWidget(srcSplit, 1);
    capv->addStretch(1);
