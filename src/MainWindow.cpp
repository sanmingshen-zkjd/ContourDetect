...
void MainWindow::updateLeftVisualDashboard() {
  ...
  auto fill=[&](QCustomPlot* p, int metricIdx, const QString& yLabel){
    ...
    p->clearItems();
    int gi = 0;
    std::unordered_map<int, std::unordered_map<int, double>> idKeyToY;
    for (int id : showIds) {
      QVector<double> x, y;
      if (!target_meas_rows_.empty()) {
    ...
    if (curIdx >= 0) {
      ...
      int focusId = selected_target_id_;
      if (focusId < 0 && !showIds.empty()) focusId = *showIds.begin();
      if (focusId >= 0) {
        auto itId = idKeyToY.find(focusId);
        if (itId != idKeyToY.end()) {
          auto itY = itId->second.find(curIdx);
          if (itY != itId->second.end() && std::isfinite(itY->second)) {
            ...
            txt->setText(QString("ID %1: %2").arg(focusId).arg(yv, 0, 'f', 3));
