...
void MainWindow::onContourMaskPointClicked(const QPointF& p) {
  ...
  if (contour_mask_mode_ == ContourMaskAdd) {
    bool useSnap = (bestContour >= 0 && bestPoint >= 0 && bestDist <= 60.0);
    ...
    if (c.empty()) return;
    int pi=0; double pd=1e18;
    for (int i=0;i<(int)c.size();++i){ double d=cv::norm(click-c[(size_t)i]); if(d<pd){pd=d;pi=i;}}
    c[(size_t)pi] = click;
    contour_mask_mode_ = ContourMaskNone;
    for (auto* v : sourceViews_) if (v) v->setToolMode(ImageViewer::PointTool);
    logLine("Modify Mask: selected contour updated.");
  } else {
    // selection mode when enabled and no active edit mode
    if (bestContour >= 0 && bestDist <= 60.0) {
      contour_mask_selected_index_ = bestContour;
    }
  }
