...
void MainWindow::buildUI() {
    ...
    btnContourRepairMode_ = new QPushButton("Contour Repair Mode", tabObj);
    btnContourRepairMode_->setCheckable(true);
    ...
    hg->addWidget(btnContourRepairMode_, 1, 0, 1, 6);
    ...
    connect(btnContourRepairMode_, &QPushButton::toggled, this, &MainWindow::onToggleContourRepairMode);
...
void MainWindow::onToggleContourRepairMode(bool on) {
  contour_repair_enabled_ = on;
  contour_repair_contour_index_ = -1;
  contour_repair_path_.clear();
  contour_repair_snap_points_.clear();
  for (auto* v : sourceViews_) {
    if (!v) continue;
    v->setPointClickCallback(on ? std::function<void(const QPointF&)>([this](const QPointF& p){ onContourRepairPointClicked(p); })
                                : std::function<void(const QPointF&)>());
    v->setToolMode(on ? ImageViewer::PointTool : ImageViewer::PanTool);
  }
  logLine(on ? "Contour repair mode enabled. Click near contour edges to build corrected path."
             : "Contour repair mode disabled.");
  onTick();
}

void MainWindow::onContourRepairPointClicked(const QPointF& p) {
  std::vector<cv::Mat> frames;
  {
    QMutexLocker locker(&frames_mutex_);
    frames = last_frames_;
  ...
  const cv::Point2f click((float)p.x(), (float)p.y());
  int chosenContour = -1;
  int chosenIndex = -1;
  double bestDist = 1e18;

  auto findNearestOnContour = [&](int contourIdx) {
    int bestIdx = -1;
    double dBest = 1e18;
    const auto& c = contours[(size_t)contourIdx];
    for (int i=0;i<(int)c.size();++i) {
      const cv::Point2f q((float)c[(size_t)i].x, (float)c[(size_t)i].y);
      const double d = cv::norm(q - click);
      if (d < dBest) { dBest = d; bestIdx = i; }
    }
    return std::pair<int,double>(bestIdx, dBest);
  };

  if (contour_repair_contour_index_ >= 0 && contour_repair_contour_index_ < (int)contours.size()) {
    auto [idx, dist] = findNearestOnContour(contour_repair_contour_index_);
    chosenContour = contour_repair_contour_index_;
    chosenIndex = idx;
    bestDist = dist;
  } else {
    for (int ci=0; ci<(int)contours.size(); ++ci) {
      auto [idx, dist] = findNearestOnContour(ci);
      if (dist < bestDist) {
        bestDist = dist;
        chosenContour = ci;
        chosenIndex = idx;
  ...
  if (chosenContour < 0 || chosenIndex < 0 || bestDist > 40.0) {
    logLine("Contour repair: click is too far from any valid edge.");
    return;
  }

  contour_repair_contour_index_ = chosenContour;
  const auto& c = contours[(size_t)chosenContour];
  const cv::Point snapped = c[(size_t)chosenIndex];
  contour_repair_snap_points_.push_back(snapped);

  if (contour_repair_snap_points_.size() == 1) {
    contour_repair_path_.clear();
    contour_repair_path_.push_back(snapped);
    onTick();
    return;
  }

  const cv::Point prev = contour_repair_snap_points_[contour_repair_snap_points_.size()-2];
  int prevIdx = -1;
  int currIdx = chosenIndex;
  for (int i=0;i<(int)c.size();++i) {
    if (c[(size_t)i] == prev) { prevIdx = i; break; }
  }
  if (prevIdx < 0) {
    contour_repair_path_.push_back(snapped);
    onTick();
  ...
  auto buildPath = [&](int a, int b, bool forward){
    std::vector<cv::Point> path;
    const int n = (int)c.size();
    int i = a;
    path.push_back(c[(size_t)i]);
    while (i != b) {
      i = forward ? ((i + 1) % n) : ((i - 1 + n) % n);
      path.push_back(c[(size_t)i]);
      if ((int)path.size() > n + 2) break;
    }
    return path;
  };
  auto pathF = buildPath(prevIdx, currIdx, true);
  auto pathB = buildPath(prevIdx, currIdx, false);
  auto lengthOf = [](const std::vector<cv::Point>& path){
    double L = 0.0;
    for (int i=1;i<(int)path.size();++i) L += cv::norm(path[(size_t)i] - path[(size_t)(i-1)]);
    return L;
  };
  const auto& bestPath = (lengthOf(pathF) <= lengthOf(pathB)) ? pathF : pathB;
  for (size_t i=1; i<bestPath.size(); ++i) contour_repair_path_.push_back(bestPath[i]);
  onTick();
}
...
void MainWindow::onTick() {
  ...
  if (!contour_repair_path_.empty()) {
    for (auto& f : vis) {
      if (f.empty()) continue;
      std::vector<std::vector<cv::Point>> arr{contour_repair_path_};
      cv::polylines(f, arr, false, cv::Scalar(0, 180, 255), 2, cv::LINE_AA);
      for (const auto& sp : contour_repair_snap_points_) {
        cv::circle(f, sp, 3, cv::Scalar(0, 0, 255), cv::FILLED, cv::LINE_AA);
      }
    }
  }
