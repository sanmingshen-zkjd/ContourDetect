...
void MainWindow::buildUI() {
    ...
    for (auto* b : {btnAddMaskContour_, btnModifyMaskContour_, btnDeleteMaskContour_}) if (b) b->setCheckable(true);
...
void MainWindow::onStartAddMaskContour() {
  const bool nextOn = btnAddMaskContour_ && btnAddMaskContour_->isChecked();
  if (btnAddMaskContour_) btnAddMaskContour_->setChecked(nextOn);
  if (btnModifyMaskContour_) btnModifyMaskContour_->setChecked(false);
  if (btnDeleteMaskContour_) btnDeleteMaskContour_->setChecked(false);
  contour_mask_mode_ = nextOn ? ContourMaskAdd : ContourMaskNone;
  if (contour_mask_mode_ == ContourMaskNone) contour_mask_working_contours_.clear();
  contour_mask_frame_key_ = -1;
  for (auto* v : sourceViews_) if (v) {
    v->setPointClickCallback(nextOn ? std::function<void(const QPointF&)>([this](const QPointF& p){ onContourMaskPointClicked(p); })
                                    : std::function<void(const QPointF&)>());
    v->setToolMode(nextOn ? ImageViewer::PointTool : ImageViewer::PanTool);
  }
  logLine(nextOn ? "Add Mask enabled: click empty area to add a new contour." : "Mask edit mode disabled.");
  onTick();
}

void MainWindow::onStartModifyMaskContour() {
  const bool nextOn = btnModifyMaskContour_ && btnModifyMaskContour_->isChecked();
  if (btnModifyMaskContour_) btnModifyMaskContour_->setChecked(nextOn);
  if (btnAddMaskContour_) btnAddMaskContour_->setChecked(false);
  if (btnDeleteMaskContour_) btnDeleteMaskContour_->setChecked(false);
  contour_mask_mode_ = nextOn ? ContourMaskModify : ContourMaskNone;
  if (contour_mask_mode_ == ContourMaskNone) contour_mask_working_contours_.clear();
  contour_mask_frame_key_ = -1;
  for (auto* v : sourceViews_) if (v) {
    v->setPointClickCallback(nextOn ? std::function<void(const QPointF&)>([this](const QPointF& p){ onContourMaskPointClicked(p); })
  ...
  logLine(nextOn ? "Modify Mask enabled: click near detected contour to edit it." : "Mask edit mode disabled.");
  onTick();
}

void MainWindow::onStartDeleteMaskContour() {
  const bool nextOn = btnDeleteMaskContour_ && btnDeleteMaskContour_->isChecked();
  if (btnDeleteMaskContour_) btnDeleteMaskContour_->setChecked(nextOn);
  if (btnAddMaskContour_) btnAddMaskContour_->setChecked(false);
  if (btnModifyMaskContour_) btnModifyMaskContour_->setChecked(false);
  contour_mask_mode_ = nextOn ? ContourMaskDelete : ContourMaskNone;
  if (contour_mask_mode_ == ContourMaskNone) contour_mask_working_contours_.clear();
  contour_mask_frame_key_ = -1;
  for (auto* v : sourceViews_) if (v) {
    v->setPointClickCallback(nextOn ? std::function<void(const QPointF&)>([this](const QPointF& p){ onContourMaskPointClicked(p); })
  ...
  logLine(nextOn ? "Delete Mask enabled: click an existing contour to remove it." : "Mask edit mode disabled.");
  onTick();
}

void MainWindow::onContourMaskPointClicked(const QPointF& p) {
  if (contour_mask_mode_ == ContourMaskNone) return;
  ...
  auto nearestContourAndPoint = [&](int& outContour, int& outPoint, double& outDist) {
    outContour = -1; outPoint = -1; outDist = 1e18;
    for (int ci=0; ci<(int)contour_mask_working_contours_.size(); ++ci) {
      const auto& c = contour_mask_working_contours_[(size_t)ci];
      if (c.empty()) continue;
      for (int i=0; i<(int)c.size(); ++i) {
        const double d = cv::norm(click - c[(size_t)i]);
        if (d < outDist) { outDist = d; outContour = ci; outPoint = i; }
      }
    }
  };

  if (contour_mask_mode_ == ContourMaskAdd) {
    for (const auto& c : contour_mask_working_contours_) {
      if (c.empty()) continue;
      const double inside = cv::pointPolygonTest(c, click, false);
      if (inside >= 0.0) {
        logLine("Add Mask rejected: click must be in area without detected contour.");
        return;
      }
    }
    ...
    const int R = 12;
    for (int k=0; k<24; ++k) {
      const double a = (2.0 * std::acos(-1.0) * k) / 24.0;
      newContour.emplace_back((int)std::round(click.x + R * std::cos(a)), (int)std::round(click.y + R * std::sin(a)));
    }
    contour_mask_working_contours_.push_back(std::move(newContour));
    logLine("Add Mask: new contour added on current frame.");
  } else if (contour_mask_mode_ == ContourMaskModify) {
    int ci=-1, pi=-1; double dist=1e18;
    // Prefer contour hit by inside test; then snap to nearest edge point.
    for (int k=0; k<(int)contour_mask_working_contours_.size(); ++k) {
      const auto& c = contour_mask_working_contours_[(size_t)k];
      if (c.empty()) continue;
      if (cv::pointPolygonTest(c, click, false) >= 0.0) { ci = k; break; }
    }
    if (ci >= 0) {
      const auto& c = contour_mask_working_contours_[(size_t)ci];
      for (int i=0;i<(int)c.size();++i) {
      ...
      nearestContourAndPoint(ci, pi, dist);
    }
    if (ci < 0 || pi < 0) {
      logLine("Modify Mask: click near an existing contour to modify it.");
      return;
    }
    contour_mask_working_contours_[(size_t)ci][(size_t)pi] = click;
    logLine("Modify Mask: contour point updated on current frame.");
  } else if (contour_mask_mode_ == ContourMaskDelete) {
    int chosen = -1;
    for (int ci=0; ci<(int)contour_mask_working_contours_.size(); ++ci) {
      const auto& c = contour_mask_working_contours_[(size_t)ci];
      if (c.empty()) continue;
      const double inside = cv::pointPolygonTest(c, click, false);
      if (inside >= 0.0) { chosen = ci; break; }
    }
    if (chosen < 0) {
      int ci=-1, pi=-1; double dist=1e18;
      nearestContourAndPoint(ci, pi, dist);
      if (ci >= 0 && dist <= 60.0) chosen = ci;
    }
    if (chosen < 0) {
      logLine("Delete Mask: click on a contour to delete it.");
      return;
    }
    contour_mask_working_contours_.erase(contour_mask_working_contours_.begin() + chosen);
    logLine("Delete Mask: contour removed on current frame.");
  }
...
void MainWindow::onTick() {
  ...
  if (contour_mask_mode_ != ContourMaskNone && contour_mask_frame_key_ != frameIdx) {
    contour_mask_working_contours_.clear();
    contour_mask_frame_key_ = -1;
    contour_mask_mode_ = ContourMaskNone;
    if (btnAddMaskContour_) btnAddMaskContour_->setChecked(false);
