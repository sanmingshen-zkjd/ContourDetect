...
void MainWindow::onContourMaskPointClicked(const QPointF& p) {
  ...
  if (contour_mask_mode_ == ContourMaskAdd) {
    ...
    if (dist < 8.0) {
      logLine("Add Mask rejected: click is too close to an existing contour edge.");
      return;
    }
    ...
    for (auto& f : vis) {
      ...
      for (const auto& am : analyzed_measures_by_frame_[frameIdx]) {
        ...
        if (std::abs(mm.m00) > 1e-9) {
          ...
          cv::putText(f, std::to_string(particleIdx++), c + cv::Point2f(4.f,-4.f), cv::FONT_HERSHEY_SIMPLEX, 0.55, cv::Scalar(0,255,255), 2, cv::LINE_AA);
        }
      }
    ...
    for (auto& f : vis) {
      ...
      for (const auto& c : analyzed_contours_) {
        ...
        if (std::abs(mm.m00) > 1e-9) {
          ...
          cv::putText(f, std::to_string(particleIdx++), ctr + cv::Point2f(4.f,-4.f), cv::FONT_HERSHEY_SIMPLEX, 0.55, cv::Scalar(0,255,255), 2, cv::LINE_AA);
        }
      }
      if (!show.empty()) cv::drawContours(f, show, -1, cv::Scalar(0,255,0), 1);
    }
  ...
  if (contour_mask_frame_key_ == frameIdx && !contour_mask_working_contours_.empty()) {
    for (auto& f : vis) {
      ...
      for (int i=0; i<(int)contour_mask_working_contours_.size(); ++i) {
        ...
        cv::Point2f c((float)(mm.m10/mm.m00), (float)(mm.m01/mm.m00));
        cv::putText(f, std::to_string(i+1), c + cv::Point2f(4.f,-4.f), cv::FONT_HERSHEY_SIMPLEX, 0.55, cv::Scalar(255,220,0), 2, cv::LINE_AA);
      }
    }
  }
